...
- Vita Companion 1.06 active on an awake Vita connected to the same trusted
  private network.
- The VitaDevDeploy agent already installed and paired with a local Ed25519
  private key as described in the [deployment guide](../../deploy/README.md).
...
Vita. FTP, Companion, and GDB default to ports 1337, 1338, and 1234. The
configure command also accepts `--ftp-port`, `--companion-port`, and
`--gdb-port` when a local setup uses different ports. The selected GDB port is
compiled into the sample and used by both symbol capture and VS Code, so the
device and desktop cannot silently disagree.
...
1. validates the ignored local profile and the full Windows VitaSDK/MSYS2
   runtime, preventing the recurring `cc1.exe` missing-DLL dialogs;
2. builds an isolated, unoptimized debug ELF and VPK under `build/` without
   reusing the repository's feature-sensitive object files;
3. records a build-identity receipt for that exact ELF/VPK pair;
4. closes only a stale `UVDBDEMO1` instance, if one is running;
5. signs, transfers, installs, and launches the VPK with VitaDevDeploy;
6. verifies the installed `eboot.bin`, uses the first real RSP connection to
   capture current ASLR/module addresses, and detaches cleanly;
7. leaves the application thread itself at a second deterministic debugger stop
...
There is deliberately no TCP readiness probe for the configured GDB port. VitaDebugger has a
single-client listener; the first successful socket is always used as an actual
RSP session so a health check cannot consume the debugger connection.
...
LAN because the underlying Vita Companion FTP/command transports are not
