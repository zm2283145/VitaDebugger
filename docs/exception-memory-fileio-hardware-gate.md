...
This is a local preparation and bounded hardware procedure for the curated
exception, memory-transaction, and File-I/O safety work. Build candidates only
from an exact clean commit and record the ELF/VPK SHA-256 values before any
hardware run. Do not use a dirty worktree or substitute binaries whose hashes
differ from those recorded values.
...
No candidate binaries, manifests, or hardware results are committed to this
repository. Build KuBridge at its unchanged checkout and the kernel companion
first. From a clean VitaDebugger worktree, record the exact commit and build
each diagnostic variant into a separate local scratch directory:
...
$Candidate = '<ABSOLUTE-LOCAL-CANDIDATE-DIRECTORY>'
C:\msys64\usr\bin\make.exe clean
C:\msys64\usr\bin\make.exe package `
  UVDB_KERNEL_THREAD_CONTROL=1 UVDB_HARDWARE_SAFETY_GATE=1
New-Item -ItemType Directory -Path $Candidate
Copy-Item .\test.elf, .\uvdb-test.vpk -Destination $Candidate
git rev-parse HEAD | Set-Content "$Candidate\commit.txt"
Get-FileHash -Algorithm SHA256 "$Candidate\test.elf"
Get-FileHash -Algorithm SHA256 "$Candidate\uvdb-test.vpk"
```

Repeat the clean build with
`UVDB_SAFETY_GATE_NULL_PREDECESSOR_TYPE=0`, `1`, and `2` into three additional
scratch directories. Keep the predecessor and NULL-predecessor files separate:
...
Before using any candidate, require a clean worktree, confirm `commit.txt`
matches the intended commit, and independently re-check both hashes. Store
hardware results under a new timestamped directory beside the candidate.
...
$Candidate = '<ABSOLUTE-LOCAL-PREDECESSOR-CANDIDATE-DIRECTORY>'
$Evidence = '<ABSOLUTE-LOCAL-EVIDENCE-DIRECTORY>'
...
hashes exactly match the recorded values. Any unexpected core dump, title exit,
...
Use the predecessor candidate and substitute its two recorded hashes:

```powershell
py -3 .\tools\gdb_exception_memory_fileio_gate.py `
  --host $VitaIp `
  --elf "$Candidate\test.elf" `
  --vpk "$Candidate\uvdb-test.vpk" `
...
1. Close `SLRS00001`, wait two seconds, install/launch the selected VPK, and
   verify the recorded hashes and both READY lines.
