...
| Hardware data watchpoints (`Z2`/`Z3`/`Z4`) | Guarded encoder and kernel-session engine under development; first monolithic disabled-register probe rebooted before its final journal and support is not advertised | Identify the failing operation with the staged read-only ladder, then pass a disabled-register probe, enabled watchpoint trap/restore test, all-core/context isolation, and watchdog cleanup |
| Hardware execution breakpoints (`Z1`) | Planned beyond KVDB's documented `Z2`-`Z4` handlers | Pass the same staged hardware gates, then advertise only the number of validated slots |
