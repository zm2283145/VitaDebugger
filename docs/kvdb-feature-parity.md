...
| Thread enumeration, selection, names, and state | Hardware-tested discovery feeds a bounded kernel-authoritative inventory (with cooperative name annotations); exact `Hg`/`Hc`, `vCont;c;s`, fail-closed selection, and deterministic foreign-thread stop attribution pass earlier live hardware tests. The production lease lifecycle has generation-qualified renewal-failure publication, and its real host translation unit passes 8,192 deterministic fake-kernel cycles covering ownership exclusion, late threads, fake-kernel plus test-build one-shot renew/end failures, watchdog expiry, fresh-stop recovery, stale completions, disconnect, restart, and cleanup. The opt-in application control binds one skipped kernel call to the exact stop token/generation, reports its outcome, and is absent from normal builds. The foreign-step host foundation requires complete prepare/execute/restore/verify callbacks and retains a rollback obligation on cleanup failure, but no Vita provider is wired. A one-shot retail 3.65 gate proves the separate first-breakpoint contention retry publishes `SIGTRAP`, dispatches the queued status query, and replies successfully | Run separate reviewed hardware gates for controlled lease failures, broader stepping, register mutation, disconnect, and rollback injection. Do not enable isolated foreign execution until a documented provider proves every capability-model ownership requirement |
