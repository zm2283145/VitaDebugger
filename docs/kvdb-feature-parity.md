...
| Thread enumeration, selection, names, and state | Hardware-tested discovery feeds a bounded kernel-authoritative inventory (with cooperative name annotations); exact `Hg`/`Hc`, `vCont;c;s`, fail-closed selection, and deterministic foreign-thread stop attribution pass earlier live hardware tests. The production lease lifecycle now has generation-qualified renewal-failure publication, and its real host translation unit passes 2,048 deterministic fake-kernel cycles covering ownership exclusion, late threads, renew/end failures, watchdog expiry, fresh-stop recovery, stale completions, disconnect, restart, and cleanup. Two ABI-v1.14 retail attempts failed to report the first persistent `step_target` breakpoint after process continue. A third retained-trace run proved the application reached the verified runtime breakpoint and isolated a sticky-failure race after unclaimed foreign state-lock contention. The minimal correction preserves fatal self/predecessor paths while allowing only an unclaimed unchanged fault to redispatch; the production-translation-unit regression and a one-shot retail 3.65 gate now prove that the retry publishes `SIGTRAP`, dispatches the queued status query, and replies successfully | Run separate reviewed hardware gates for broader stepping, register mutation, disconnect, and controlled lease-failure injection. Do not enable isolated foreign execution until a documented provider proves every capability-model ownership requirement |
