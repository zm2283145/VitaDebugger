...
| 32 ARM/Thumb software breakpoints | Implemented; 32 slots. The transactional patch helper now has host-tested exact-byte snapshot/read-back/restoration, partial-write and verification-corruption rollback, disconnect cleanup, competing-slot ownership, retry, and retained target/module identity checks | Wire identity-bound patching only after a Vita provider can retain the exact process and module objects across validation and the following write; then inject the same failure matrix on hardware |
...
| ARM/Thumb software single-step | Broad practical decoder implemented. `Hc0` and `vCont;s:T;c` hardware-step deterministic foreign Thumb workers through distinct paths using state-dependent raw-bank selection. GDB's exact-stopped-thread positive-`Hc` hidden-breakpoint step-over passes without `E16`; representative A32 PC writers and bounded Thumb-2/A32 `LDREX`-through-`STREX` sequences also pass live GDB. The [retail 3.65 practical-gate record](hardware/gdb-step-register-exclusive-3.65.json) includes successful single stores and exact trap-byte restoration after abrupt disconnect. Host coverage now also includes transaction failure/rollback and an explicit foreign-step capability model requiring scheduler, context, trap, and rollback ownership in one generation. Non-executed Vita fixtures carry representative accepted encodings and fail-closed PC-writer, privileged-return, `BXJ`, register-shift, wait/syscall, exclusive-sequence, and IT-state hazards | Keep arbitrary foreign-thread isolation disabled: current VitaSDK headers expose per-thread debug suspend/resume and read-only snapshots, but no documented scheduler-lock or atomic resume-one/resuspend contract. Hardware-gate every new encoding and identity-bound rollback provider before promotion |
...
| Thread enumeration, selection, names, and state | Hardware-tested discovery feeds a bounded kernel-authoritative inventory (with cooperative name annotations); exact `Hg`/`Hc`, `vCont;c;s`, fail-closed selection, and deterministic foreign-thread stop attribution pass earlier live hardware tests. The production lease lifecycle now has generation-qualified renewal-failure publication, and its real host translation unit passes 2,048 deterministic fake-kernel cycles covering ownership exclusion, late threads, renew/end failures, watchdog expiry, fresh-stop recovery, stale completions, disconnect, restart, and cleanup. Two ABI-v1.14 retail attempts failed to report the first persistent `step_target` breakpoint after process continue. A third retained-trace run proved the application reached the verified runtime breakpoint: its first callback lost a foreign state-lock race and poisoned transport state, while the immediate retry completed breakpoint publication and kernel stop but exited on that stale failure before replying. The minimal host-modeled correction preserves fatal self/predecessor paths while allowing only an unclaimed unchanged fault to redispatch; a production-translation-unit regression requires the second callback to send `T05` | Run exactly one reviewed first-breakpoint retest before broader stepping or controlled failure injection. Do not enable isolated foreign execution until a documented provider proves every capability-model ownership requirement |
...
1. Promote the host-complete controlled thread renew/end failure and
   multithreaded stress matrix through a serialized retail-hardware gate.
   Keep arbitrary-foreign-thread execution disabled while researching a
   documented scheduler-locked resume-one/resuspend primitive or displaced
   stepping provider that can satisfy the complete ownership model.
...
4. Connect the host-tested restorable kernel mutation ABI to a supported setter
   ...
   hardware injected-memory/trap-rollback gates before enabling privileged
   exception returns, unsupported exclusive forms, or other unsafe instruction
   families.
