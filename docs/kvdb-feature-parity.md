...
| Thread enumeration, selection, names, and state | Hardware-tested discovery feeds a bounded kernel-authoritative inventory (with cooperative name annotations); exact `Hg`/`Hc`, `vCont;c;s`, fail-closed selection, and deterministic foreign-thread stop attribution pass live hardware tests. The production lease lifecycle now has generation-qualified renewal-failure publication, and its real host translation unit passes 2,048 deterministic fake-kernel cycles covering ownership exclusion, late threads, renew/end failures, watchdog expiry, fresh-stop recovery, stale completions, disconnect, restart, and cleanup | Run the controlled failure and long-soak lifecycle on retail hardware with serialized deployment and recovery. Do not enable isolated foreign execution until a documented provider proves every capability-model ownership requirement |
