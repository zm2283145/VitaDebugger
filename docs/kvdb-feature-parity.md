...
| Thread enumeration, selection, names, and state | Hardware-tested discovery feeds a bounded kernel-authoritative inventory (with cooperative name annotations); exact `Hg`/`Hc`, `vCont;c;s`, fail-closed selection, and deterministic foreign-thread stop attribution pass earlier live hardware tests. The production lease lifecycle now has generation-qualified renewal-failure publication, and its real host translation unit passes 2,048 deterministic fake-kernel cycles covering ownership exclusion, late threads, renew/end failures, watchdog expiry, fresh-stop recovery, stale completions, disconnect, restart, and cleanup. Two ABI-v1.14 retail attempts failed to report the first persistent `step_target` breakpoint after process continue; the host-modeled resume handoff is not hardware-validated. A retained eight-callback phase trace and evidence-preserving first-breakpoint-only gate now narrow the next diagnostic without enabling new execution behavior | Run only the retained-trace first-breakpoint diagnostic before broader stepping or controlled failure injection. Do not enable isolated foreign execution until a documented provider proves every capability-model ownership requirement |
