...
clean:
	...
	rm -f test-pmu-cleanup-gate-record test-pmu-cleanup-gate-record.exe test-pmu-process-event-pending test-pmu-process-event-pending.exe
...
host-test-pmu-cleanup-gate-record: test-pmu-cleanup-gate-record$(HOST_EXEEXT)
...
host-test-screen:
	$(MAKE) -C screen host-test HOST_CC=$(HOST_CC) PYTHON="$(HOST_PYTHON)"
...
test-kernel-pmu-profiler-safe-rearm$(HOST_EXEEXT): kernel/src/pmu_profiler_bridge.c kernel/src/pmu_profiler_bridge.h kernel/src/pmu_profiler_transport.c kernel/src/pmu_profiler_transport.h kernel/src/pmu_backend.c kernel/src/pmu_backend.h kernel/src/pmu_backend_host_test.h kernel/src/pmu_session.c kernel/src/pmu_session.h kernel/include/vitadebug_pmu_profiler.h profiler/src/vitaprofiler.c profiler/src/vitaprofiler_names.c profiler/src/vitaprofiler_pmu.c profiler/include/vitaprofiler_pmu.h profiler/include/vitaprofiler.h tests/host/test_kernel_pmu_profiler_bridge.c
	$(HOST_CC_RUN) $(HOST_CFLAGS) -pedantic-errors -DVD_KERNEL_ENABLE_EXPERIMENTAL_PMU_SESSION=1 -DVD_KERNEL_ENABLE_EXPERIMENTAL_PMU_PROFILER_TRANSPORT=1 -DVD_KERNEL_ENABLE_EXPERIMENTAL_PMU_PROFILER_REAL_EVENTS=1 -DVD_KERNEL_ENABLE_EXPERIMENTAL_PMU_PROFILER_SAFE_REARM=1 -DVD_KERNEL_ENABLE_EXPERIMENTAL_PMU_PROFILER_PROCESS_EVENTS=1 -DVD_TEST_EXPECT_REAL_EVENTS_COMPILED=1 -DVD_TEST_EXPECT_SAFE_REARM_COMPILED=1 -DVD_TEST_EXPECT_PROCESS_EVENTS_COMPILED=1 -DVD_PMU_BACKEND_HOST_TEST=1 -Iprofiler/include -Ikernel/include -Ikernel/src kernel/src/pmu_profiler_bridge.c kernel/src/pmu_profiler_transport.c kernel/src/pmu_backend.c kernel/src/pmu_session.c profiler/src/vitaprofiler.c profiler/src/vitaprofiler_names.c profiler/src/vitaprofiler_pmu.c tests/host/test_kernel_pmu_profiler_bridge.c -o $@
...
test-pmu-process-event-pending$(HOST_EXEEXT): kernel/src/pmu_process_event_pending.h tests/host/test_pmu_process_event_pending.c
	$(HOST_CC_RUN) $(HOST_CFLAGS) -pedantic-errors -Ikernel/src tests/host/test_pmu_process_event_pending.c $(HOST_THREAD_FLAGS) -o $@
