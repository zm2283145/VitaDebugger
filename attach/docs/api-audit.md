...
Layer 2 adds a serialized authentication-only listener, a sentinel VPK, and a
bounded public-metadata store core. The secure-storage review is a hard block
for production authentication on retail 3.65: no public/safely callable API
provides both a non-exportable Ed25519 key isolated from ordinary/co-resident
SceShell code and an independent durable monotonic rollback floor. Public
`sceIo` path checks cannot establish a trusted handle-bound commit. There is no
Vita private-key or persistence backend, and the VPK exits without
