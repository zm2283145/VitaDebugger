...
| Debugger RSP completion | [Terminal record](docs/hardware/rsp-completion-terminal-3.65.json), [curation scope](docs/rsp-curation-record.md) | **RSP NOT QUALIFIED**; selected admission/reset observations passed, but the completion connection received neither ACK nor response and no hardware pass is claimed |
| Debugger client admission gate | [Admission record](docs/hardware/debugger-client-admission-retail-3.65.json) | Silent, HTTP, partial, and bad-checksum probes did not consume the listener; GDB Ctrl-C, detach/reconnect, and pending-candidate shutdown passed |
