...
static int vp_stream_write_v2_chunk(struct vp_stream_writer* writer,
                                    uint16_t type,
                                    ...
                                    size_t payload_size)
{
    ...
    result = vp_stream_write(writer, header, sizeof(header));
    if (result != VP_RESULT_OK)
        return result;
    if (payload_size != 0u) {
        result = vp_stream_write(writer, payload, payload_size);
        if (result != VP_RESULT_OK)
            return result;
    }
...
static void vp_stream_encode_stats_v2(
    const struct vp_stream_writer* writer, const struct vp_stats* ring,
                           ...
                           uint64_t stream_start_us)
{
    ...
    if (result != VP_RESULT_OK)
        return result;
    writer->wire_version = VP_WIRE_VERSION;
    ...
    const struct vp_stream_v2_session* session)
{
    ...
    writer->wire_version = VP_STREAM_V2_VERSION;
                           ...
                           size_t max_events, size_t* events_written)
{
    ...
    if (writer->wire_version == VP_STREAM_V2_VERSION) {
        size_t capacity =
            writer->dictionary_buffer_capacity / VP_WIRE_EVENT_SIZE;
        ...
        size_t limit = max_events < capacity ? max_events : capacity;
        while (count < limit &&
                               ...
                               struct vp_stream_writer_stats* stats)
{
    ...
    stats->wire_version = writer->wire_version;
    ...
    const struct vp_stream_v2_chunk_view* chunk)
{
    ...
    case VP_STREAM_V2_CHUNK_DICTIONARY:
        return vp_name_wire_validate_le(
            payload, chunk->header.payload_size, NULL);
