...
| --- | --- | --- | --- |
| Peer disconnect during a live lease | Cross-layer fixture and hardware stage use a 5 s lease, require exact `VP_ERROR_IO`, require another nonzero-identity authenticated PMU read, then require successful PMU close, socket close, and network teardown | Socket failure remains visible and cannot be confused with the lease timeout path; zero identities and partial cleanup are rejected; PMU stays owned until close; close restores the complete snapshot; only then may a later real event open | Required |
