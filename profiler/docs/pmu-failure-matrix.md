...
| --- | --- | --- | --- |
| Peer disconnect during a live lease | Cross-layer fixture and hardware stage use a 5 s lease, inject a disconnected-send error, require another authenticated PMU read before bounded socket shutdown/close, then perform authenticated PMU close | Socket failure remains visible and cannot be confused with the lease timeout path; PMU stays owned until close; close restores the complete snapshot; only then may a later real event open | Required |
